...
def calc_true_pdf(dist_code):
    ...
    grid_n = 256 if dist.dim == 2 else 35
    points = grid_points(dist.dim)
    return dist.dim, dist.pdf(points)


def calc_class_ise_hd(dim, wde, true_pdf):
...
def class_ise_hd_fname(dist_code, wave, j0, j1, sample_size, start, block_size):
    ...
    the_dir = '%(pdir)s/clwe/%(wave)s/clw-%(j0)02d-%(j1str)s/%(num)05d' % \
               dict(pdir=pdir, wave=wave, j0=j0, j1str=j1str, num=sample_size)
    ensure_dir(the_dir)
    return '%(the_dir)s/ise-%(block_size)03d-%(start)03d.csv' % \
           dict(the_dir=the_dir, block_size=block_size, start=start)


class ClassIseWriter(object):
    def __init__(self, dist_code, wave, j0, j1, sample_size, start, block_size):
        self.fname = class_ise_hd_fname(dist_code, wave, j0, j1, sample_size, start, block_size)
        ...
        print('ClassIseWriter: Generating %s' % self.fname)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.fh is not None:
            ...
            print('ClassIseWriter: %s done in %f secs' % (self.fname, elapsed_time))
...
def main(dist_code, wave, j0, j1, sample_size, start, block_size):
    """
    ...
    with ClassIseWriter(dist_code, wave, j0, j1, sample_size, start, block_size) as writer:
        for i in range(start, start + block_size):
            ...
            t0 = datetime.now()
            wde = SimpleWaveletDensityEstimator(wave, j0 = j0, j1 = j1)
            wde.fit(sample)
            elapsed_time = (datetime.now() - t0).total_seconds()
            ise, hd = calc_class_ise_hd(dim, wde, true_pdf)
...
if __name__ == "__main__":
    ...
    main(args.code, args.wave, args.j0, args.j1, args.n, args.start, args.block_size)
